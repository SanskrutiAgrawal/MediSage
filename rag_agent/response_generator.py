...
class ResponseGenerator:
    ...
    def _build_prompt(
        ...
        # Build the prompt
        main_prompt = f"""You are a medical assistant providing accurate information based on verified medical sources.
        ...
        final_prompt_for_llm = f"""<|system|>
        You are a helpful medical assistant. Answer the user's question based *only* on the provided context. Follow all formatting instructions.</s>
        <|user|>
        {main_prompt}</s>
        <|assistant|>
        """
    ...
    def generate_response(
        ...
        try:
            ...
            # LLM call
            response_obj = self.response_generator_model.invoke(prompt)
            ...
            # Append images (optional but recommended)
            if picture_paths:
                response_text += "\n\n##### Reference images:"
